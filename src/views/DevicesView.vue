...
const handleAssign = async () => {
    ...
    try {
        ...
        message.value = `Device ${deviceId.value} assigned successfully.`
        deviceId.value = ''
    } catch (error) {
        console.error(error)
        isError.value = true
        message.value = 'Failed to assign device.'
    } finally {
...
const handleUnassign = async () => {
    ...
    try {
        ...
        message.value = `Device ${deviceId.value} unassigned successfully.`
        deviceId.value = ''
    } catch (error) {
        console.error(error)
        isError.value = true
        message.value = 'Failed to unassign device.'
    } finally {
        isLoading.value = false
    }
}
...
<template>
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold mb-6 text-zinc-900 dark:text-zinc-50">Manage Devices</h1>

        <div class="bg-white dark:bg-zinc-900 shadow rounded-lg p-6 border border-zinc-200 dark:border-zinc-800">
            <div class="mb-4">
                <label for="deviceId" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Device
                    ID</label>
